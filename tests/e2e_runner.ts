...
async function dump(page: any, label: string) {
    ...
    } catch (e) { log(`WARN could not screenshot dump ${label}: ${(e as Error).message}`); }
    try {
        const html = await page.content();
        fs.writeFileSync(path.join(DUMPS_DIR, `${n}-${safe}.html`), html);
    } catch (e) { log(`WARN could not html-dump ${label}: ${(e as Error).message}`); }
    log(`[dump] ${n}-${safe}`);
}
...
async function clickTrustAuthor(page: any): Promise<boolean> {
    for (let i = 0; i < 20; i++) {
        const clicked = await page.evaluate(() => {
            ...
            const btn = buttons.find(b => /trust author/i.test(b.innerText))
...
async function takeAndCompareScreenshot(page: puppeteer.Page, name: string) {
    await dump(page, name);
    const screenshotPath = path.join(__dirname, '..', `${name}.png`);
    await page.screenshot({ path: screenshotPath });

    const goldenPath = path.join(__dirname, '..', 'tests', '__goldens__', `${name}.png`);
    ...
    if (numDiffPixels > 500) { // threshold to allow for slight rendering artifacts (e.g. blinking cursor)
...
function regenGoldensIfRequested() {
    ...
    log(`E2E_REGEN_GOLDENS=1: removed ${removed} old golden(s); this run will save fresh ones into tests/__goldens__.`);
}
...
async function run() {
    ...
    const pluginPath = path.join(vaultPath, '.obsidian', 'plugins', 'hanzi-practice');

    console.log('Setting up vault at:', vaultPath);
    fs.mkdirSync(pluginPath, { recursive: true });

    // Copy built plugin files
    fs.copyFileSync(path.join(__dirname, '..', 'main.js'), path.join(pluginPath, 'main.js'));
    fs.copyFileSync(path.join(__dirname, '..', 'manifest.json'), path.join(pluginPath, 'manifest.json'));
    ...
    if (!browser) {
        throw new Error(`Could not connect to Obsidian after ${connectRetries} retries (remote-debugging port ${DEBUG_PORT} never became reachable)`);
    }
    ...
    try {
        ...
        while (Date.now() < deadline) {
            ...
            lastTargetsDump = targets.map((t: any) => `${t.type()}:${t.url()}`).join('\n  ');
            ...
            for (const t of pageTargets) {
                ...
                } catch (e) { /* execution context may be tearing down */ }
        ...
        if (!page) {
            ...
            throw new Error('Could not find a loaded Obsidian workspace page (layoutReady never became true)');
        }
        ...
        log(trusted ? 'Clicked "Trust author and enable plugins".' : 'No trust prompt appeared (already trusted).');
        await delay(1500);
        ...
        console.log('STEP 3: Enabling community plugins + Hanzi Practice plugin...');
        const pluginEnabled = await page.evaluate(async () => {
            const app = (window as any).app;
            try { await app.plugins.setEnable(true); } catch (e) { /* already on */ }
            try { await app.plugins.enablePluginAndSave('hanzi-practice'); }
            catch (e) { try { await app.plugins.enablePlugin('hanzi-practice'); } catch (e2) {} }
            // Give onload a beat to register commands.
            await new Promise(r => setTimeout(r, 500));
            return !!(app.plugins.plugins && app.plugins.plugins['hanzi-practice'])
                && !!app.commands.commands['hanzi-practice:add-hanzi-character'];
        });
        if (!pluginEnabled) {
            await dump(page, 'STEP3-plugin-not-enabled');
            throw new Error('Hanzi Practice plugin did not enable / register its commands.');
        }
        ...
        await page.evaluate(() => {
            ...
            setTimeout(() => (window as any).app.setting.openTabById('community-plugins'), 200);
        });
        await delay(2000);
        ...
        for (let idx = 0; idx < charsToAdd.length; idx++) {
            ...
            for (let i = 0; i < 5; i++) {
                addResult = await page.evaluate(() => {
                    return (window as any).app.commands.executeCommandById('hanzi-practice:add-hanzi-character');
                });
                if (addResult) break;
                await delay(1000);
            }
            if (!addResult) {
                throw new Error('Command hanzi-practice:add-hanzi-character failed to execute');
            }
            
            await delay(1000); // wait for modal
            await page.waitForSelector('.modal input[type="text"]', { timeout: 5000, visible: true });
        ...
        if (!dupOpened) {
            throw new Error('Command hanzi-practice:add-hanzi-character failed to execute for duplicate test');
        }
        await delay(1000);
        await page.waitForSelector('.modal input[type="text"]', { timeout: 5000, visible: true });
        ...
        const dupErrorShown = await page.evaluate(() => {
            const err = document.querySelector('.modal .hanzi-add-error') as HTMLElement | null;
            const modalStillOpen = !!document.querySelector('.modal input[type="text"]');
            return modalStillOpen && !!err && err.style.display !== 'none' && err.textContent!.trim().length > 0;
        });
        if (!dupErrorShown) {
            throw new Error('Duplicate character did not surface an inline error in the still-open modal!');
        }
        ...
        for (const char of charsToAdd) {
            if (!practiceMd.includes(char)) {
                throw new Error(`Character ${char} not found in hanzi-practice-words.md!`);
            }
        }
        ...
        if (!haoLine || !haoLine.includes('\t')) {
            throw new Error(`Expected 好 to have cached pinyin/def (tab-separated); got: ${JSON.stringify(haoLine)}`);
        }
        ...
        console.log(`Verified characters + cached pinyin/def in words file (好 -> ${haoPinyin}).`);

        // STEP 6: Run the test command so you can test your hanzi skill
        console.log('STEP 6: Running test command...');
        await page.evaluate(() => {
            (window as any).app.commands.executeCommandById('hanzi-practice:open-hanzi-practice');
        });
        await delay(2000);

        // The practice view must live in the center (main) area, not a sidebar.
        const inCenterPane = await page.evaluate(() => {
            const leaf = document.querySelector('.workspace-leaf-content[data-type="hanzi-practice-view"]');
            if (!leaf) return false;
            return !!leaf.closest('.mod-root') && !leaf.closest('.mod-left-split') && !leaf.closest('.mod-right-split');
        });
        if (!inCenterPane) {
        ...
        for (let i = 0; i < 30; i++) {
            dictReady = await page.evaluate(() => {
                ...
                const toneButtons = view.querySelectorAll('.tone-selector .pinyin-btn-container button');
                return !!meaning && (meaning.textContent || '').trim().length > 0 && toneButtons.length > 0;
            }).catch(() => false);
            if (dictReady) break;
            await delay(1000);
        }
        if (!dictReady) {
            ...
            throw new Error('Practice view did not render the meaning + tone selector (dictionary failed to load)!');
        }
        ...
        if (!fs.existsSync(historyMdPath)) {
            throw new Error(`hanzi-practice-history.md not found at ${historyMdPath}`);
        }
        ...
        await page.evaluate(() => {
            (window as any).app.setting.open();
            setTimeout(() => (window as any).app.setting.openTabById('hanzi-practice'), 200);
        });
        await delay(1000);
        ...
        if (page) { try { await dump(page, 'FAILURE'); } catch (e2) {} }
    } finally {
