import { encodeStrokeData, StrokeDataReader, CharMedians } from '../src/data/stroke_codec';

describe('Stroke data codec (HZS1)', () => {
  const hao: CharMedians = [
    [[282, 788], [307, 769], [327, 733], [264, 465]],
    [[139, 512], [305, 535], [368, 549]],
  ];
  const han: CharMedians = [
    [[100, -124], [200, 900]], // exercise the extremes of the coordinate range
  ];
  ...
  it('round-trips characters exactly', () => {
    const reader = roundTrip(new Map([['好', hao], ['汉', han]]));
  ...
  it('handles negative and boundary int16 coordinates', () => {
    ...
    expect(reader.get('汉')![0]).toEqual([[100, -124], [200, 900]]);
  });
  ...
  it('rejects blobs without the magic header', () => {
    expect(() => new StrokeDataReader(new Uint8Array([1, 2, 3, 4, 0, 0, 0, 0]))).toThrow(/magic/);
  });
