...
## Readings (pinyin) are NOT stored on cards

Cards hold the text and the meaning — never the reading. `开车⇥to drive (a car)`, not
`开车⇥kāichē — to drive (a car)`. The reading is printed above each character at render time
from the **character ledger** (below), which means it can disappear as you learn.

- Existing cards with an embedded reading are fixed by the command **"Migrate Cards: Remove
  Embedded Pinyin"**. It strips a `pīnyīn — ` prefix from the text fields and leaves every
  other field — id included — untouched, so no card loses its review history. It only strips a
  prefix that really looks like a reading (no hanzi, pinyin-legal, has a tone mark), so
  `Paris — capital of France` is safe. Running it twice does nothing the second time.

## The character ledger (`hanzi-character-progress.md`)

Generated by **"Sync Character Progress"**: every hanzi that appears in any card, with its
dictionary reading and meaning. It is three things at once:

1. **The reading source.** Cards annotate their Chinese from this file, so the practice view
   never loads the 10MB dictionary.
2. **A practiceable bank** (`Characters`). The characters your study packs introduced can be
   drilled with the stroke quiz like any other bank.
3. **A progress note.** The file opens with a table of level / reviews / average per
   character; the card lines follow it.
...
| --- | --- | --- | --- | --- | --- |
| 车 | chē | car/vehicle | 5 | 3 | 5.0 |

车⇥che1⇥car/vehicle/CL:輛|辆[liang4]⇥4cc383d7⇥0⇥Characters
```

**Levels and when the reading disappears.** A character's level is the rounded mean of its
scores, and it cannot reach level 4 until it has at least 3 reviews (one lucky sentence is not
proof). At **level 4+** the character renders WITHOUT its reading — the line above it stays
reserved, so the card does not reflow the moment you learn something.

**Where the reviews come from.** Grading any card also credits every tracked character in it:
answering `你喜欢开车吗？` is evidence about 开 and 车, not just about that card. Those credits
appear in the history file as `<id> 车 (via card <cardId>): 5`. A re-sync never rewrites an
existing line, so ids (and therefore history) and any definition you edited by hand survive.

The reading chosen for a character is the first CEDICT sense that is not a surname, variant or
colloquialism — 车 gets `che1` (car), not `Che1` (a surname), and 吗 gets `ma5` (the question
particle), not `ma2` ("(coll.) what?").
